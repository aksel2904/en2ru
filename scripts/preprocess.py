...
def main(cfg: DictConfig):
    ...
    # 1. Сплит
    with open(en_path, 'r') as f: en = [line.strip().lower() for line in f]
    with open(ru_path, 'r') as f: ru = [line.strip().lower() for line in f]
    df = pd.DataFrame({'en': en, 'ru': ru})
    train_val, test = train_test_split(df, test_size=cfg.split.test_size, random_state=cfg.split.seed)
    train, val = train_test_split(train_val, test_size=cfg.split.val_size / (1 - cfg.split.test_size), random_state=cfg.split.seed)
