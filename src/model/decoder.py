...
class Decoder(nn.Module):
    ...
    def __init__(self, output_dim, emb_dim, enc_hid_dim, dec_hid_dim, attention):
        ...
        self.rnn = nn.GRU(
            input_size=emb_dim + enc_hid_dim * 2,
            hidden_size=dec_hid_dim
        )

        self.out = nn.Linear(
            in_features=attention.attn_in + emb_dim,
    ...
    def forward(self, input, decoder_hidden, encoder_outputs):
        ...
        weighted = self.weighted_encoder(decoder_hidden, encoder_outputs)  # [1, batch, enc_hid_dim * 2]
        rnn_input = torch.cat((embedded, weighted), dim=2)          # [1, batch, emb_dim + enc_hid_dim * 2]

        output, hidden = self.rnn(rnn_input, decoder_hidden.unsqueeze(0))  # output: [1, batch, dec_hid_dim]
        ...
        prediction = self.out(torch.cat((output, weighted, embedded), dim=1))  # [batch, output_dim]
